...
test_that("streaming univariate statistics", {
	...
	sx <- s_mean(x, na.rm=TRUE); sy <- s_mean(y, na.rm=TRUE)
	expect_equal(mean(xy, na.rm=TRUE), as.numeric(stat_c(sx, sy)))
	
	sx <- s_var(x, na.rm=TRUE); sy <- s_var(y, na.rm=TRUE)
...
test_that("streaming matrix statistics", {
	...
	sx <- s_rowstats(x, "sd"); sy <- s_rowstats(y, "sd")
	...
	sx <- s_rowstats(x > 0, "any"); sy <- s_rowstats(y > 0, "any")
	expect_equal(as.logical(apply(xy > 0, 1, any)), as.logical(stat_c(sx, sy)))

	sx <- s_rowstats(x > 0, "all"); sy <- s_rowstats(y > 0, "all")
	expect_equal(as.logical(apply(xy > 0, 1, all)), as.logical(stat_c(sx, sy)))

	sx <- s_rowstats(x > 0, "nnzero"); sy <- s_rowstats(y > 0, "nnzero")
	...
	sx <- s_rowstats(x, "mean", na.rm=TRUE); sy <- s_rowstats(y, "mean", na.rm=TRUE)
	expect_equal(as.numeric(apply(xy, 1, mean, na.rm=TRUE)), as.numeric(stat_c(sx, sy)))

	sx <- s_rowstats(x, "var", na.rm=TRUE); sy <- s_rowstats(y, "var", na.rm=TRUE)
