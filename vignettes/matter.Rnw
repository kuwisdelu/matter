...
xsub <- matter_mat(offset=c(40, 120, 200, 280, 360),
            extent=rep(5,5), filepaths=filepaths(x))
x[6:10,]
xsub[]
@
...
<<>>=
x2 <- matter_vec(offset=80, extent=10, filepaths=filepaths(x))
y3 <- matter_vec(offset=160, extent=10, filepaths=filepaths(y))
cbind(x2, y3)[]
cbind(x[,2], y[,3])
@
...
\item \Robject{filepaths}: the paths to the files used by the \Robject{atoms} objects
