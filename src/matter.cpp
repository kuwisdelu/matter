...
void Matter :: writeMatrixRows(SEXP i, SEXP value) {
    ...
    switch(S4class()) {
        case 2:
            for ( int col = 0; col < ncols; col++ ) {
                Atoms atoms(data(col));
                atoms.writeAt<RType>(pValue + col * nrows, pRow, nrows, files());
            }
            break;
        case 3:
            for ( int l = 0; l < nrows; l++ ) {
...
void Matter :: writeMatrixCols(SEXP j, SEXP value) {
    ...
    switch(S4class()) {
        ...
        case 3:
            for ( int row = 0; row < nrows; row++ ) {
                Atoms atoms(data(row));
                atoms.writeAt<RType>(pValue + row, pCol, ncols, files(), nrows);
            }
            break;
    }
}
    ...
    switch(S4class()) {
        case 2:
            for ( int l = 0; l < ncols; l++ ) {
                ...
                index_type col = static_cast<index_type>(pCol[l]);
                Atoms atoms(data(col));
                atoms.writeAt<RType>(pValue + l * nrows, pRow, nrows, files());
            }
            break;
        case 3:
            for ( int l = 0; l < nrows; l++ ) {
                ...
                index_type row = static_cast<index_type>(pRow[l]);
                Atoms atoms(data(row));
                atoms.writeAt<RType>(pValue + l, pCol, ncols, files(), nrows);
            }
            break;
    }
}
