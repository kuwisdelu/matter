...
sparse_old_mat <- function(data, datamode = "double", nrow = 0, ncol = 0,
	...
	if ( rowMaj ) {
		mclass <- "sparse_old_matr"
		if ( missing(ncol) && !missing(keys) )
			ncol <- length(keys)
	} else {
		mclass <- "sparse_old_matc"
		if ( missing(nrow) && !missing(keys) )
			nrow <- length(keys)
	}
	...
	x <- new(mclass,
		...
		ops=NULL,
		keys=keys,
		tolerance=as_sparse_old_mat_tolerance(tolerance),
		combiner=as_sparse_old_mat_combiner(combiner))
	if ( !missing(data) && !is.list(data) )
		x[] <- data
	x
}
...
as_sparse_old_mat_tolerance <- function(tolerance) {
	tol <- tolerance[1]
...
setMethod("describe_for_display", "sparse_old_mat", function(x) {
	desc1 <- paste0("<", x@dim[[1]], " row, ", x@dim[[2]], " column> ", class(x))
	desc2 <- paste0("sparse ", x@datamode[2], " matrix")
	paste0(desc1, " :: ", desc2)
})

setMethod("preview_for_display", "sparse_old_mat", function(x) {
	hdr <- preview_matrix_data(x)
	if ( is(x, "sparse_old_matc") ) {
...
setAs("matrix", "sparse_old_mat",
	function(from) sparse_old_mat(from, datamode=typeof(from), dimnames=dimnames(from)))

setAs("array", "sparse_old_mat",
	function(from) sparse_old_mat(as.matrix(from), datamode=typeof(from), dimnames=dimnames(from)))

as.sparse <- function(x, ...) as(x, "sparse_old_mat")

is.sparse <- function(x) is(x, "sparse_old_mat")

setAs("sparse_old_mat", "matrix", function(from) from[])

setMethod("as.matrix", "sparse_old_mat", function(x) as(x, "matrix"))

setMethod("keys", "sparse_old_mat", function(object) object@keys)

setReplaceMethod("keys", "sparse_old_mat", function(object, value) {
	object@keys <- value
	if ( validObject(object) )
		object
})

setReplaceMethod("keys", "sparse_old_matc", function(object, value) {
...
setReplaceMethod("keys", "sparse_old_matr", function(object, value) {
	if ( length(value) != nrow(object) ) {
...
setMethod("tolerance", "sparse_old_mat", function(object) object@tolerance)

setReplaceMethod("tolerance", "sparse_old_mat", function(object, value) {
	object@tolerance <- as_sparse_old_mat_tolerance(value)
	object
})

setMethod("combiner", "sparse_old_mat", function(object) object@combiner)

setReplaceMethod("combiner", "sparse_old_mat", function(object, value) {
	object@combiner <- as_sparse_old_mat_combiner(value)
	object
})
...
getSparseMatrixElements <- function(x, i, j, drop=TRUE) {
	...
	if ( is(x, "sparse_old_matc") ) {
		rowMaj <- FALSE
	} else if ( is(x, "sparse_old_matr") ) {
		rowMaj <- TRUE
	} else {
		stop("unrecognized 'sparse_old_mat' subclass")
	}
	sorted <- FALSE
	if ( is.null(keys(x)) ) {
...
setMethod("[",
	c(x = "sparse_old_mat", i = "ANY", j = "ANY", drop = "ANY"),
	function(x, i, j, ..., drop) {
...
setMethod("[",
	c(x = "sparse_old_mat", i = "ANY", j = "ANY", drop = "NULL"),
	function(x, i, j, ..., drop) {
		narg <- nargs() - 1 - !missing(drop)
		if ( !missing(i) && narg == 1 )
			stop("linear indexing not supported")
...
setReplaceMethod("[",
	c(x = "sparse_old_mat", i = "ANY", j = "ANY", value = "ANY"),
...
setMethod("combine_by_rows", c("sparse_old_matr", "sparse_old_matr"), function(x, y, ...) {
	if ( ncol(x) != ncol(y) )
...
setMethod("combine_by_cols", c("sparse_old_matc", "sparse_old_matc"), function(x, y, ...) {
	if ( nrow(x) != nrow(y) )
...
setMethod("t", "sparse_old_matc", function(x)
{
	class(x) <- "sparse_old_matr"
	x@dim <- rev(x@dim)
...
setMethod("t", "sparse_old_matr", function(x)
{
	class(x) <- "sparse_old_matc"
	x@dim <- rev(x@dim)
	x@dimnames <- rev(x@dimnames)
	if ( validObject(x) )
		x
...
setMethod("%*%", c("sparse_old_matc", "matrix"), function(x, y)
{
	rightMatrixMult(x, y, useOuter=TRUE)
})

setMethod("%*%", c("sparse_old_matr", "matrix"), function(x, y)
{
	rightMatrixMult(x, y, useOuter=FALSE)
})

setMethod("%*%", c("matrix", "sparse_old_matc"), function(x, y)
{
	leftMatrixMult(x, y, useOuter=FALSE)
})

setMethod("%*%", c("matrix", "sparse_old_matr"), function(x, y)
{
